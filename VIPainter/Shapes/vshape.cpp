...
QVector<VPoint> VShape::getRect(bool isExtended)
{
    QVector<VPoint> points;
    double x = 0, y = 0;
    if(isExtended){
        x=crDis/(transform.map(VPoint(1,0))-transform.map(VPoint(0,0)));
        y=crDis/(transform.map(VPoint(0,1))-transform.map(VPoint(0,0)));
    }
...
void VShape::drawCR(QPainter * painter, const VTransform &trans, double scale, bool isExtended)
{
    Q_UNUSED(scale);
    painter->save();
    QVector<VPoint> points = this->getRect(isExtended);
